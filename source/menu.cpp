...
static int MenuGameList()
{
	...
	while(menu == MENU_NONE)
	{
		...
		if ( info.selection >= 0 ){
			if (WindowPrompt("Download Codes", "Are you sure you want to download these codes?", "Yes", "No"))
			{
				if (DownloadCodes(info.selection, info.type, GameList) == 0)
				{
					WindowPrompt("Download Codes", "Codes downloaded successfully!", "Cool", NULL);
				}
				else
				{
					WindowPrompt("Holy Crap", "Something went horribly wrong!", "Aww :(", NULL);
				}
